...
class Resource {
	...
	out(compilation) {
		let { publicPath: webpackOutputPublicPath = '' } = compilation.compiler.options.output;
		...
		const htmlOutputURL = url.resolve(webpackOutputPublicPath, this.options.filename);
		let newNodes = [];
		...
		const outToNewNodes = (fileName, fileContent) => {
			...
			// webpackOutputPublicPath = '/' 时，不必计算相对路径，否则开发时会影响二级页面的使用
			if(webpackOutputPublicPath != '/'){
		...
		const outToOutputNodes = () => {
			if (newNodes.length > 0) {
				...
				if (compilation.chunks.findIndex(chunk => chunk.name === this.chunkName) > -1) {
					...
					// 就认为这是一个直接导入本地文件的资源。
					// 提前过滤调部分 URL 地址，提高性能，例如 http://url.cn/1.js  //url.cn/1.js
					if (this.chunkName && !/(https?:)?\/\/.+\..+.*/.test(this.chunkName)) {
						// 相对于模版文件去解析资源文件地址
						const filePath = path.resolve(path.dirname(this.options.template || '.'), this.chunkName);
						try {
							// 去读取对应的文件
